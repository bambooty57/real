  ...
  const handleExportExcel = async () => {
    try {
      ...
      worksheet.columns = [
        ...
        { header: '우편수신', key: 'canReceiveMail', width: 10 },
        { header: '보유장비', key: 'equipments', width: 50 },
        ...
        worksheet.addRow({
          ...
          detailCrops: (() => {
            ...
            return Object.entries(mainCrop)
              ...
              .flatMap(([key, values]) => {
                const cropType = key.replace('Details', '');
                return Array.isArray(values) ? values.map(value => cropDisplayNames[value] || value) : [];
              })
              .filter(Boolean)
              .join(', ');
          })(),
